...
EXPERIMENT = Path('/data/experiments/value-ablation-20260906')
import lerobot
...
class VisionOnlyModel(ValueModel):
    """No Gemma, task tokens, state tokens or replacement state MLP."""
    def __init__(self,cfg):
        from transformers import AutoModel, AutoImageProcessor
        from lerobot.values.pistar06.modeling_pistar06 import _extract_vision_feature_size, _resolve_image_size, _resolve_norm_stats, _maybe_enable_gradient_checkpointing
        torch.nn.Module.__init__(self); self.cfg=cfg; self.model_dtype=torch.float32
        self.vision_encoder=AutoModel.from_pretrained(cfg.vision_repo_id,torch_dtype=torch.float32,local_files_only=True)
        processor=AutoImageProcessor.from_pretrained(cfg.vision_repo_id,use_fast=True,local_files_only=True)
        self.image_resolution=_resolve_image_size(processor); mean,std=_resolve_norm_stats(processor)
        self.register_buffer('image_mean',torch.tensor(mean).view(1,1,3,1,1),persistent=False)
        self.register_buffer('image_std',torch.tensor(std).view(1,1,3,1,1),persistent=False)
        dim=cfg.fusion_hidden_dim
        self.image_projector=torch.nn.Sequential(torch.nn.Linear(_extract_vision_feature_size(self.vision_encoder),dim),torch.nn.GELU(),torch.nn.Dropout(cfg.dropout))
        self.final_norm=torch.nn.LayerNorm(dim)
        self.value_head=torch.nn.Sequential(torch.nn.Linear(dim,dim),torch.nn.GELU(),torch.nn.Dropout(cfg.dropout),torch.nn.Linear(dim,cfg.num_bins))
        if cfg.use_gradient_checkpointing: _maybe_enable_gradient_checkpointing(self.vision_encoder)

    def forward(self,images,image_attention_mask):
        x=self._preprocess_images(images,image_attention_mask)
        b,n=x.shape[:2]; features=self._encode_images(x.reshape(b*n,*x.shape[2:]).to(self.model_dtype))
...
def main():
    ...
    protocol.update(variant=args.variant,experiment='fixed-horizon-ablation-v1',training_stop='EXACT steps, no early stopping; final checkpoint is primary comparison',early_stop='disabled',selection='final-step endpoint is primary; intermediate minima are secondary',runner_sha256=digest(__file__),pure_vision_removes_state=args.variant=='vision_only',language_receives_state=args.variant in ('full','frozen_language','frozen_both'),lr_schedule=f'warmup{args.warmup} then cosine to min_lr AT fixed final step')
    ...
    if protocol_exists[0]:
        ...
        if not args.evaluate_checkpoint:
            ...
            for k in ('steps','warmup','eval_every','eval_frames','variant','seed','peak_lr','min_lr'):
    ...
    for step,batch in enumerate(train_loader,start+1):
        model.train()
        lr = args.peak_lr*step/max(1,args.warmup) if step<=args.warmup else args.min_lr+0.5*(args.peak_lr-args.min_lr)*(1+math.cos(math.pi*(step-args.warmup)/max(1,args.steps-args.warmup)))
