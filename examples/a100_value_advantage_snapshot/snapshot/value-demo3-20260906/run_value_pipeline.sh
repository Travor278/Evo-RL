...
test ! -e runs/targetenv-8gpu-smoke-v2
"$python_bin" -m torch.distributed.run --standalone --nproc_per_node=8 -- value_experiment.py \
  --task piperx_insert_copper_screw --run targetenv-8gpu-smoke-v2 \
  --steps 3 --batch-size 4 --workers 2 --eval-every 3 --eval-frames 3 --warmup 3 --smoke \
  >> targetenv-8gpu-smoke-v2.log 2>&1
grep -q '^SMOKE_SAVE_RELOAD_OK' targetenv-8gpu-smoke-v2.log
