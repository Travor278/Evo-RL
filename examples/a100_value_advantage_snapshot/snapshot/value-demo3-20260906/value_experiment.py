...
def main():
    ...
    if args.evaluate_checkpoint:
        ...
        # Inference-only diagnostics run serially at this existing evaluation boundary.
        # The training branch, optimizer, sampler, checkpoint selection and upstream sources are unchanged.
        if (BASE/'extended-review-request.json').exists():
            from extended_value_review import run_extended_review
            run_extended_review(model,manifest,train,test,train_eval,args,tokenizer,device,centers,rank,world,out)
