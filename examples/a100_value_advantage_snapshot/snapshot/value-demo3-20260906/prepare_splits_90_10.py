...
ARCHIVE = BASE.parent / 'rwrl_so101_piperx_duration_matched_seed1000_v1.tar.gz'
TASKS = {
...
with tarfile.open(ARCHIVE) as archive:
    for task, (root, expected_n, expected_frames) in TASKS.items():
        ...
        n_test = math.floor(expected_n * .1 + .5)
        groups = {'test': ranked[:n_test], 'train': ranked[n_test:]}
        ...
        report = {
            ...
            'split_algorithm': 'sha256 identity rank, namespace evorl-value-demo-split-v1; rounded 10% test, remainder train',
            ...
            'success_label_status': 'all selected episodes terminal success, explicitly confirmed by user 2026-09-06',
            'test_usage': 'periodic holdout evaluation and checkpoint selection per user request; not an untouched final test',
