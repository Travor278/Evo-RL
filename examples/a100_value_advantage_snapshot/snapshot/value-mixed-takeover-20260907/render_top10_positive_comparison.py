"""Three frozen-model, frame-synchronized videos with episode-local top10% & A>0 colors.
...
OUT=ROOT/'video-top10_positive-episode11-three-models';OUT.mkdir(exist_ok=True)
...
def render(spec):
    ...
    target=OUT/f'{key}_episode11_top10_positive.mp4';validation=OUT/f'{key}_validation.json'
    if target.exists():
    ...
    v=np.array(result['value_common_scale']);a=np.array(result['advantage_full']);selected=np.array(result['selected_top10_positive'])
    ...
    p.text((810,577),'RED: TOP10% & A>0',font=f16,fill=red);p.text((1010,577),'WHITE: REST',font=f16,fill=white)
    ...
    for i,(main,left,right) in enumerate(frames):
        ...
        d.text((980,492),'SELECTED: TOP10% +' if selected[i] else ('TERMINAL' if i==T else 'NOT SELECTED'),font=f14,fill=red if selected[i] else white)
        ...
        d.text((35,926),f'Per-video TOP10% AND A>0 | C=150, Z=15337 | {result["selected_count"]}/{T} nonterminal frames | offline / future data',font=f14,fill=muted)
...
def main():
    ...
    for key,(step,training_scale,checkpoint) in configs.items():
        ...
        k=math.ceil(.1*T);chosen=np.argsort(-a[:T],kind='stable')[:k];chosen=chosen[a[chosen]>0]
        top=np.zeros(n,dtype=bool);top[chosen]=True;assert top.sum()==len(chosen) and np.all(a[top]>0)
        put(OUT/f'{key}_timeline.json',{'episode':EP,'step':step,'checkpoint':checkpoint,'training_scale':training_scale,'display_scale':Z,'predicted_value_native':native.tolist(),'value_common_scale':v.tolist(),'advantage_full':a.tolist(),'time_value_delta':delta.tolist(),'explicit_penalty':penalty.tolist(),'selected_top10_positive':top.tolist(),'selected_count':int(top.sum()),'selection_cap':k,'positive_count':int((a[:T]>0).sum()),'selection_min_advantage':float(a[chosen].min()) if len(chosen) else None,'human_control':d.masks[EP].tolist(),'takeover_onset_frames':(np.flatnonzero(d.events[EP])+1).tolist(),'selection':'episode-local, per model; all nonterminal frames including human control; descending full A; ceil(10%) with A>0; stable ties by frame index','comparison_note':'Same physical episode, input cameras, timestamps, evaluation C150/Z15337/n50 and terminal bootstrap0. Demo-only trained on time cost without HIL; mixed trained on time+takeover costs. Scale conversion does not erase target/data/training differences. Demo old8x4 vs mixed4x8, global32.'})
    ...
    put(OUT/'comparison_manifest.json',{'episode':EP,'frames':n,'fps':FPS,'models':configs,'shared_value_limits':[vlo,vhi],'shared_advantage_limits':[-amp,amp],'red_rule':'within episode each model highest10% intersect strictly positive A; not all A>0','render_specs':specs})
