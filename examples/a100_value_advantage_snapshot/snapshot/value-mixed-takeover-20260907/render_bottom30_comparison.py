"""Three frozen-model, frame-synchronized videos with episode-local bottom30% colors.
...
OUT=ROOT/'video-bottom30-episode11-three-models';OUT.mkdir(exist_ok=True)
...
def render(spec):
    ...
    target=OUT/f'{key}_episode11_bottom30.mp4';validation=OUT/f'{key}_validation.json'
    if target.exists():
    ...
    cut_target=OUT/f'{key}_episode11_bottom30_selected_only.mp4';cut_part=cut_target.with_suffix('.part.mp4');assert not cut_target.exists() and not cut_part.exists()
    v=np.array(result['value_common_scale']);a=np.array(result['advantage_full']);selected=np.array(result['selected_bottom30'])
    ...
    p.text((810,577),'RED: BOTTOM30%',font=f16,fill=red);p.text((1010,577),'WHITE: REST',font=f16,fill=white)
    ...
    cut_encoder=av.open(str(cut_part),'w');cut_stream=cut_encoder.add_stream('libx264',rate=FPS)
    ...
    for i,(main,left,right) in enumerate(frames):
        ...
        d.text((980,492),'SELECTED: BOTTOM30%' if selected[i] else ('TERMINAL' if i==T else 'NOT SELECTED'),font=f14,fill=red if selected[i] else white)
        ...
        d.text((35,926),f'Per-video BOTTOM30% | C=150, Z=15337 | {result["selected_count"]}/{T} nonterminal frames | offline / future data',font=f14,fill=muted)
        ...
        if selected[i]:
            ...
            d.text((25,12),'BOTTOM30% SELECTED ONLY | CUTS / DISCONTINUOUS | ORIGINAL TIME AT RIGHT',font=f20,fill=red)
    ...
    cut_encoder.close()
    cut_decoded=0
    with av.open(str(cut_part)) as con:
    ...
    os.replace(cut_part,cut_target)
    decoded=0
    ...
    put(validation,{'status':'ok','frames':n,'fps':FPS,'seconds':n/FPS,'bytes':target.stat().st_size,'full_decode_passed':True,'selected_only_path':str(cut_target),'selected_only_frames':cut_decoded,'selected_only_full_decode_passed':True,'selected_count':int(selected.sum()),'eligible_frames':T,'selection_min_advantage':result['selection_min_advantage'],'color_rule':'lowest30% by ascending raw full advantage, stable ties by frame; nonterminal frames including human-control segments','value_limits':spec['value_limits'],'advantage_limits':spec['advantage_limits']})
...
def main():
    ...
    for key in ['mixed1500','demo1500']:
        path=ROOT/'video-top20-episode11-three-models'/f'{key}_native_predictions.json'
        assert path.exists(),'Existing inference cache required; do not run new inference'
        if path.exists():
    ...
    for key,(step,training_scale,checkpoint) in configs.items():
        ...
        k=math.ceil(.3*T);chosen=np.argsort(a[:T],kind='stable')[:k]
        top=np.zeros(n,dtype=bool);top[chosen]=True;assert top.sum()==k
        put(OUT/f'{key}_timeline.json',{'episode':EP,'step':step,'checkpoint':checkpoint,'training_scale':training_scale,'display_scale':Z,'predicted_value_native':native.tolist(),'value_common_scale':v.tolist(),'advantage_full':a.tolist(),'time_value_delta':delta.tolist(),'explicit_penalty':penalty.tolist(),'selected_bottom30':top.tolist(),'selected_count':int(top.sum()),'selection_cap':k,'positive_count':int((a[:T]>0).sum()),'selected_negative_count':int((a[top]<0).sum()),'selection_max_advantage':float(a[chosen].max()),'selection_min_advantage':float(a[chosen].min()) if len(chosen) else None,'human_control':d.masks[EP].tolist(),'takeover_onset_frames':(np.flatnonzero(d.events[EP])+1).tolist(),'selection':'episode-local, per model; all nonterminal frames including human control; ascending full A; ceil(30%); no sign filter; stable ties by frame index','comparison_note':'Same physical episode, input cameras, timestamps, evaluation C150/Z15337/n50 and terminal bootstrap0. Demo-only trained on time cost without HIL; mixed trained on time+takeover costs. Scale conversion does not erase target/data/training differences. Demo old8x4 vs mixed4x8, global32.'})
        edges=np.diff(np.r_[False,top,False].astype(int));starts=np.flatnonzero(edges==1);stops=np.flatnonzero(edges==-1)
        segments=[{'start_frame':int(lo),'end_frame_exclusive':int(hi),'start_s':float(lo/FPS),'end_s_exclusive':float(hi/FPS),'frames':int(hi-lo),'mean_advantage':float(a[lo:hi].mean()),'min_advantage':float(a[lo:hi].min())} for lo,hi in zip(starts,stops)]
        put(OUT/f'{key}_selected_segments.json',{'episode':EP,'segments':segments,'segment_count':len(segments),'selected_count':k,'negative_count':int((a[top]<0).sum())})
        print('SELECTION',key,k,'negative',int((a[top]<0).sum()),'segments',len(segments),'threshold',float(a[chosen].max()),flush=True)
    ...
    put(OUT/'comparison_manifest.json',{'episode':EP,'frames':n,'fps':FPS,'models':configs,'shared_value_limits':[vlo,vhi],'shared_advantage_limits':[-amp,amp],'red_rule':'within episode each model lowest30% ascending A, no sign filter','render_specs':specs})
