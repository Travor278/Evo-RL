...
OUT=ROOT/'video-extremes10-chunk50-episode11-three-models'
FPS=30;HORIZON=50
...
def render(spec):
    basekey=spec['key'];group=spec['group'];key=f'{basekey}_{group}';group_label='TOP10% & A>0' if group=='top10' else 'BOTTOM10%';data=json.loads((SOURCE/f'{basekey}_timeline.json').read_text())
    a=np.array(data['advantage_full']);v=np.array(data['value_common_scale']);human=np.array(data['human_control']);n=len(a)
    k=math.ceil(.1*(n-1));ranked=np.argsort(-a[:-1] if group=='top10' else a[:-1],kind='stable')[:k]
    if group=='top10':ranked=ranked[a[ranked]>0]
    anchors=np.sort(ranked);assert len(anchors)>0
    if group=='top10':assert np.all(a[anchors]>0)
...
def main():
    ...
    jobs=[dict(spec,group=group) for group in ['top10','bottom10'] for spec in manifest['render_specs']]
