...
SOURCE=ROOT/'video-top30_positive-episode11-three-models'
OUT=ROOT/'video-positive-chunk50-episode11-three-models'
FPS=30;HORIZON=50
...
def render(spec):
    key=spec['key'];data=json.loads((SOURCE/f'{key}_timeline.json').read_text())
    a=np.array(data['advantage_full']);v=np.array(data['value_common_scale']);human=np.array(data['human_control']);n=len(a)
    anchors=np.flatnonzero(data['selected_top30_positive']);assert np.all(a[anchors]>0)
    ...
    p.text((74,744),'YELLOW TICKS: POSITIVE SELECTED ANCHORS',font=f14,fill=yellow)
    ...
    def compose(images,i,t,mode,example_index=None):
        ...
        title='ALL SELECTED 50-STEP WINDOWS | OVERLAPS MERGED' if mode=='union' else f'COMPLETE CHUNK {example_index+1}/12 | SAME ANCHOR THROUGH ALL 50 STEPS'
        ...
        d.text((18,495),'CANDIDATE / NOT TRAINED',font=f14,fill=muted)
        d.text((980,438),f'Original {i/FPS:.2f}s',font=f16,fill=white)
        d.text((980,465),'HUMAN CONTROL' if human[i] else 'AUTONOMOUS',font=f16,fill=yellow if human[i] else muted)
        ...
        d.text((25,926),f'{label}: t={t} ({t/FPS:.2f}s) | Current V={v[i]:+.4f} | 1 chunk = 50 actions | origin TOP30% & A>0',font=f14,fill=muted)
        return canvas

    w=writer(OUT/f'{key}_positive_chunk50_union.mp4');written=0;cache={}
    ...
    summary={'key':key,'selected_anchors':len(anchors),'union':union,'examples':examples,'example_count':len(picks),'short_tail_windows':sum(w['video_frames_available']<50 for w in windows),'controller_crossing_windows':sum(w['crosses_controller_boundary'] for w in windows)}
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=3,mp_context=mp.get_context('spawn')) as pool:results=list(pool.map(render,manifest['render_specs']))
