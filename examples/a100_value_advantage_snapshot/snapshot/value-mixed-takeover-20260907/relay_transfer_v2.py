...
def plan(root,work):
    ...
    atomic(work/'relay_plan.json',result)
...
def receive(root,work):
    lock=(work/'relay_receive.lock').open('a');fcntl.flock(lock,fcntl.LOCK_EX|fcntl.LOCK_NB)
    p=json.loads((work/'relay_plan.json').read_text());expected={r['path']:r for r in p['missing']}
    ...
    report=dict(status='ok',files=m['files'],bytes=m['bytes'],records=m['records'],root=str(root),
                transport='Mac-relay-missing-only-v2',at=time.time())
    atomic(work/'transfer_verified.json',report)
    print('TRANSFER_COMPLETE SHA_ALL_260_OK',flush=True)
...
def relay():
    ...
    remote=ROOT+'/relay_transfer_v2.py'
    ...
    try:
        sender=subprocess.Popen(ssh+[source,shlex.join(['python3','/home/zhaobo/value_relay_transfer_v2_20260907.py','send','--root',SOURCE])],stdin=subprocess.PIPE,stdout=subprocess.PIPE)
