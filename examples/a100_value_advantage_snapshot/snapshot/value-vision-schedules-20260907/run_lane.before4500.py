...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('lane',choices=['constant','cosine']);ap.add_argument('--smoke-only',action='store_true');a=ap.parse_args()
    ...
    try:
        ...
        for steps in ([8000] if a.lane=='constant' else [3000,6000]):run(steps)
