...
if [[ "$MODE" == "acp" ]]; then
  JOB=v2sam-ego2exo-official-v2-r5-4gpu
  OUTPUT=$TARGET/checkpoints/${JOB}-acp-policy
else
  JOB=v2sam-ego2exo-official-v3-r5-4gpu
...
"$VENV/bin/python" -m torch.distributed.run --standalone --nproc_per_node=4 --master_port=$PORT "$OLD_ROOT/preflight/nccl_smoke.py"
echo "FORMAL_POLICY_NCCL_GATE_OK ranks=4"
...
payload = {
 ...
 'job':job,'mode':mode,'world_size':4,
 'batch_size_per_rank':16,'global_batch_size':64,'workers_per_rank':4,'default_prefetch_factor':2,'total_prefetch_samples':512,
 'steps':10000,'save_steps':[1000,5000,10000],'seed':20260902,'target_hil_fraction':0.25,'mixed_ram_stage_seconds':stage_seconds,
...
ARGS=(
  ...
  --batch_size=16 --num_workers=4 --eval_freq=0 --save_checkpoint=true --log_freq=5 --seed=20260902 --wandb.enable=false
...
if [[ "$MODE" == "acp" ]]; then
  MODE_ARGS=(--acp.enable=true --acp.indicator_field="$ACP_FIELD" --acp.apply_mask_field="$ACP_MASK_FIELD" --acp.indicator_dropout_prob=0.30)
else
  MODE_ARGS=(--acp.enable=false)
fi
ARGS+=("${MODE_ARGS[@]}" --steps=10000 --save_freq=0 '--save_steps=[1000,5000,10000]' --output_dir="$OUTPUT" --job_name="$JOB")

echo "FORMAL_POLICY_TRAIN_START name=$JOB mode=$MODE utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
train_start=$(date +%s)
"$VENV/bin/python" -m accelerate.commands.launch \
  --multi_gpu --num_processes=4 --num_machines=1 --mixed_precision=bf16 --main_process_port=$((PORT+1)) \
...
while kill -0 "$train_pid" 2>/dev/null; do
  ...
  set +e
  decision=$("$VENV/bin/python" "$SUPERVISOR" --log "$LOG" --marker FORMAL_POLICY_TRAIN_START --target-steps 10000 --max-projected-seconds 43200 --min-metrics 2)
  decision_rc=$?
  set -e
...
test "$train_seconds" -le 43200
test "$(readlink "$OUTPUT/checkpoints/last")" = 010000
for step in 001000 005000 010000; do
...
sha256sum "$OUTPUT/checkpoints/001000/pretrained_model/model.safetensors" "$OUTPUT/checkpoints/005000/pretrained_model/model.safetensors" "$OUTPUT/checkpoints/010000/pretrained_model/model.safetensors" > "$TARGET/manifests/${JOB}-checkpoint-sha256.txt"
