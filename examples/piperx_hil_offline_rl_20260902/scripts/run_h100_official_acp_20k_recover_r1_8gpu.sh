...
JOB=v2sam-ego2exo-official-v2-20k-r1-recover-r1-8gpu
OUTPUT=$TARGET/checkpoints/v2sam-ego2exo-official-v2-20k-r1-8gpu-acp-policy
RESUME_CHECKPOINT=$OUTPUT/checkpoints/015000
RESUME_CONFIG=$RESUME_CHECKPOINT/pretrained_model/train_config.json
RAM_ROOT=/dev/shm/v2sam-ego2exo-official-v2-20k-r1-8gpu
...
test -d "$OUTPUT" && test -f "$RESUME_CONFIG"
test "$(readlink "$OUTPUT/checkpoints/last")" = 015000
test ! -e "$OUTPUT/checkpoints/020000"
test ! -e "$MANIFEST" && test ! -e "$REPORT"
test "$(python -c "import json;print(json.load(open('$RESUME_CHECKPOINT/training_state/training_step.json'))['step'])")" = 15000
...
python - "$MANIFEST" "$0" "$JOB" "$MODE" "$DATASET_MANIFEST" "$ACP_GATE" "$RESUME_CHECKPOINT/pretrained_model" "$stage_seconds" <<'PY'
...
payload = {
 ...
 'recovery':{'reason':'step-20000 checkpoint save failed because gpfs_hdd was full','resume_step':15000,'resume_optimizer_scheduler_rng':True},
...
"$VENV/bin/python" -m accelerate.commands.launch \
  ...
  --module lerobot.scripts.lerobot_train "${RESUME_ARGS[@]}" &
train_pid=$!
...
report.write_text(
    '# 8×H100 ACP 20k checkpoint recovery\n\n'
    f'- Job: `{job}`\n'
    f'- Mode: `{mode}`\n'
    '- Steps: `20000`\n'
    '- Resume checkpoint: `15000` with optimizer, scheduler, and RNG state\n'
